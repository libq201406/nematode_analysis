...
# ================== 读取本地图片并嵌入 ==================
# 只要本地文件夹里有 banner.png，它就会自动变成代码的一部分
try:
    with open("banner.png", "rb") as f:
        banner_base64 = base64.b64encode(f.read()).decode()
    banner_html = f'<img src="data:image/png;base64,{banner_base64}" style="width:100%; border-radius: 12px;">'
except:
    banner_html = ""
...
<style>
    ...
    div.stButton > button {
        background-color: #1e4d8f; color: white; border-radius: 30px; border: none;
        padding: 12px 40px; font-size: 18px; font-weight: 600; transition: 0.3s;
        display: block; margin: 20px auto;
    }
    div.stButton > button:hover { background-color: #153a6b; box-shadow: 0 4px 12px rgba(30,77,143,0.3); }
</style>
...
# ================== 页面顶部 ==================
# 显示图片，如果本地没有图片，就显示一行提示
if banner_html:
    st.markdown(banner_html, unsafe_allow_html=True)
else:
    st.warning("⚠️ 本地未找到 banner.png，请确保它与 web.py 在同一文件夹下。")
    ...
    if current_files != st.session_state['last_uploaded_file']:
        ...
        with st.spinner('正在分析中，请稍候...'):
            for uploaded_file in uploaded_files:
                ...
                vis_img[mask_final_red > 0] = [0, 0, 255]
                vis_img_rgb = cv2.cvtColor(vis_img, cv2.COLOR_BGR2RGB)

                results_data.append({
                    "文件名": uploaded_file.name, "身体面积(像素)": pixels_body,
        ...
        if results_data:
            ...
            st.download_button(label="📥 一键下载全部统计数据 (CSV)", data=csv_data,
                               file_name="area_analysis_results.csv", mime="text/csv")